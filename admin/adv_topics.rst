...
플랫폼적인 정책을 고민하신다면 최소한 1개월간 서비스 지표를 모니터링하는 것을 권장한다.
...
디스크 캐싱모드에서 병목의 주요원인은 디스크 I/O이지만 쉽게 간과되고 잘 드러나지 않는다.

이런 이유로 최대 인덱싱개수는 128GB 기준인 ``45,390,095`` 개로 제한되어 있다.
...
이 제한은 Disk I/O가 저하되지 않는 수준에서, 1천만개 단위로 조정할 것을 권장한다.
