...
각 모듈이 동작하는 시점을 명확히 이해해야 안전한 서비스 운영이 가능하다.
...
`가상호스트 링크`_ 는 HTTP 응답을 가로채 재분기를 시키는 메카니즘이다.


.. note::

   `URL 전처리`_ 와 `가상호스트 링크`_ 가 다음과 같이 사용되는 경우가 종종 있다.
   
   .. figure:: img/avdhostpoint5.png
      :align: center
   
   `가상호스트 링크`_ 는 `URL 전처리`_ 에 의해 상위계층에 있기 때문에 모든 가상호스트는 변조된 경로를 사용한다.
