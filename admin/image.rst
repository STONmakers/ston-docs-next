...
원본보다 과도하게 이미지를 크게 확대하는 경우 문제 소지가 있다.

-  커진 이미지로 인한 전송량 증가
-  과도한 이미지 프로세싱에 따른 성능저하
-  메모리 과다사용

특히 ``Animated GIF`` 포맷인 경우 낱개의 이미지 모두를 처리해야 하기 때문에 의도치 않은 문제가 발생될 수 있다.
...
.. note::
   ...
   ``StretchOut`` 속성과 ``ExceptionGIF`` 속성의 연계를 통해 GIF 이미지에 대해서만 확대요청 처리를 허용하지 않을 수 있다. ::

      <Dims StretchOut="ON" ExceptionGIF="OFF" />

   위와 같이 설정하면 ``GIF`` 가 아닌 ``JPG`` , ``PNG`` 형식의 이미지만 처리된다.
